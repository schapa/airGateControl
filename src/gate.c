...
void Gate_SetState(const _Bool newState) {
	s_gateControl.realState = BSP_GateLedState();
	setState(newState);
}
...
void Gate_onLedStateChange(const _Bool ledState) {
	...
	if (ledState != s_gateControl.requestedState) {
		...
		if (s_gateControl.userBlockTimId) {
			...
			Timer_delete(s_gateControl.userBlockTimId);
			s_gateControl.userBlockTimId = Timer_newArmed(USER_BLOCK_TIMEOUT, false, onUserBlockTimeout, NULL);
		}
	} else {
		/* success set */
		Timer_delete(s_gateControl.sateBlockTimId);
		s_gateControl.sateBlockTimId = Timer_newArmed(DEBOUNCE_STATE_CHANGE, false, onStateBlockTimeout, NULL);
	}
}
